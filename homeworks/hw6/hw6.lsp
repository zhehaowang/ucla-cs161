...
(defun pairs-c-k (n c k)
  (cond
    ((= c k) NIL)
    (t (cons (list (- 0 (node2var n c k)) (- 0 (node2var n k k))) (pairs-c-k n (+ c 1) k)))
  ))
...
(defun at-most-one-color (n c k)
  (cond 
    ((= c k) NIL)
    (t (append (at-most-one-color n c (- k 1)) (pairs-c-k n c k)))
    )
  )
...
(defun generate-edge-clauses-helper (e c k)
  (cond 
    ((> c k) NIL)
    (t (cons (list (- 0 (node2var (first e) c k)) (- 0 (node2var (second e) c k))) (generate-edge-clauses-helper e (+ c 1) k)))
  ))
